...
**Tag soft-delete (hide).** Right-clicking a sidebar tag opens an Obsidian `Menu` (`onTagContext` in `QuickMemoView`) whose "删除标签" item **soft-deletes** the tag: it appends the tag to `settings.deletedTags` and persists via the view's `saveSettings` callback (passed from `main.ts`). The `#tag` text is **not** removed from records — the index still counts it; the tag only leaves the sidebar list because `QuickMemoView.render()` runs `index.tags()` through the pure `filterVisibleTags(tags, settings.deletedTags)` helper in `viewState.ts`. Hiding is reversible with no confirm dialog (just a Notice); restore happens in 设置 → 管理已删除标签, a `Modal` (`manageDeletedTagsDialog` in `SettingsTab.ts`) reachable from both render paths (an `addButton` in `display()` and a `SettingDefinitionAction` appended in `getSettingDefinitions()`). `MarkdownRecordRepository.removeTag` (a hard delete that rewrites the `#tag` out of every record) remains as a tested repo capability but is **no longer wired to the UI**. Note: `SettingsTab.setControlValue` mutates `plugin.settings` **in place** (not reassign) so the long-lived view's captured settings reference stays fresh after a declarative change — the view reads/writes `deletedTags` on that shared reference.
...
- **Settings tab has two render paths.** `QuickMemoSettingTab` keeps the imperative `display()` (works on all versions; `minAppVersion` is 1.7.2) **and** implements `getSettingDefinitions()` (the declarative API, 1.13.0+) so settings appear in Obsidian's unified settings search. On 1.13.0+ the framework renders the declarative definitions and does **not** call `display()`; on older versions `display()` is the fallback. Definitions live in the pure `src/settings/settingDefinitions.ts` (type-only Obsidian import → unit-tested without the runtime). The declarative path persists via `setControlValue`, which routes through `normalizeSettings` + `QuickMemoPlugin.saveSettings()` (not the base, which persists but does not rebuild the index / refresh the view) so heading/folder/format changes stay live. Keep definitions and `display()` names/descs/options in sync (the `quickMemoSettingDefinitions` test guards the keys).
